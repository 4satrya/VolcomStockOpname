Public Class ClassStockTake
    ...
    Public Function queryTransMainUN(ByVal condition As String, ByVal order_type As String) As String
        If order_type = "1" Then
            order_type = "ASC "
        ElseIf order_type = "2" Then
            order_type = "DESC "
        ...
        Else
            condition = ""
        End If
        Dim query As String = "SELECT st.id_wh_drawer, st.remark, c.id_comp,c.comp_number, c.comp_name, CONCAT(c.comp_number,' - ', c.comp_name) AS `comp`, c.address_primary,st.id_st_un_reg, st.un_reg_number, 
        st.un_reg_date, st.un_reg_by, ep.employee_code AS `prepared_by_code`, IF(up.is_external_user = 1, up.name_external, ep.employee_name) AS `prepared_by`, IF(up.is_external_user = 1, up.position_external, ep.employee_position) AS `prepared_position`, 
        st.un_reg_updated_by, e.employee_code, IF(u.is_external_user = 1, u.name_external, e.employee_name) AS employee_name, 
        st.un_reg_updated_date, st.id_report_status, rs.report_status, st.report_status_note, IFNULL(q.qty,0) AS `qty`, 'No' AS `is_select`, st.acknowledge_by, IF(uack.is_external_user = 1, uack.name_external, eack.employee_position) AS `ack_position`, st.approved_by
        FROM tb_st_un_reg st
        LEFT JOIN tb_lookup_report_status rs ON rs.id_report_status = st.id_report_status
        LEFT JOIN tb_m_user u ON u.id_user = st.un_reg_updated_by
        LEFT JOIN tb_m_employee e ON e.id_employee = u.id_employee 
        LEFT JOIN tb_m_user up ON up.id_user = st.un_reg_by
        LEFT JOIN tb_m_employee ep ON ep.id_employee = up.id_employee 
        LEFT JOIN tb_m_comp c ON c.id_drawer_def = st.id_wh_drawer
        LEFT JOIN tb_m_user uack ON uack.id_user = st.acknowledge_by
        LEFT JOIN tb_m_employee eack ON eack.id_employee = uack.id_employee 
        LEFT JOIN (
            SELECT COUNT(*) AS `qty`, id_st_un_reg FROM tb_st_un_reg_det GROUP BY id_st_un_reg
        ) q ON q.id_st_un_reg = st.id_st_un_reg
        WHERE st.id_st_un_reg>0 "
        query += condition + " "
        query += "ORDER BY st.id_st_un_reg " + order_type
        Return query
    End Function

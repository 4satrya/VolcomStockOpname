Public Class ClassStockTake
    ...
    Public Function queryTransMain(ByVal condition As String, ByVal order_type As String) As String
        ...
        st.st_trans_updated, st.is_combine, st.id_report_status, rs.report_status, st.report_status_note, IFNULL(q.qty,0) AS `qty`, 'No' AS `is_select`, IFNULL(st.id_combine,0) AS `id_combine`, st.acknowledge_by, IF(uack.is_external_user = 1, uack.name_external, eack.employee_position) AS `ack_position`, st.approved_by
        FROM tb_st_trans st

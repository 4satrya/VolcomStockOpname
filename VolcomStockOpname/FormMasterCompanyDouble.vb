    ...
    Private Sub GCComp_KeyDown(sender As Object, e As KeyEventArgs) Handles GCComp.KeyDown
        If e.KeyCode = Keys.Enter Then
            If GVComp.FocusedRowHandle >= 0 And GVComp.RowCount > 0 Then
                'FormStock.id_drw_rsv = GVComp.GetFocusedRowCellValue("id_drawer_def").ToString
                'FormStock.TxtCodeAccRsv.Text = GVComp.GetFocusedRowCellValue("comp_number").ToString
                'FormStock.TxtNameAccRsv.Text = GVComp.GetFocusedRowCellValue("comp_name").ToString
                'FormStock.BtnViewRsv.Focus()
                Close()
            End If
        End If
    End Sub
